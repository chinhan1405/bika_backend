...
FRONTEND_REPO_NAME = "bika-frontend"
...
def npm(
    ...
    with pathlib.Path(frontend_repo_path / "package.json").open() as file:
        node_version = json.loads(file.read())["engines"]["node"]
    context.run(f"volta fetch node@{node_version}")
    with context.cd(frontend_repo_path):
        context.run(
            f"volta run --node {node_version} npm {command}",
        )
